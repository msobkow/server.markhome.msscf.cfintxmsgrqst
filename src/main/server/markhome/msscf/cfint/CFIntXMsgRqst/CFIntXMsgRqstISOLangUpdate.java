...
/*
 *	CFIntXMsgRqstISOLangUpdate XML SAX Element  implementation
 *	for ISOLang.
 */
public class CFIntXMsgRqstISOLangUpdate
	extends CFLibXmlCoreElementHandler
{
	public CFIntXMsgRqstISOLangUpdate( CFIntXMsgRqst xmsgRqst ) {
		super( xmsgRqst );
	}
	...
	throws SAXException
	{
		...
		try {
			...
			assert qName.equals( "RqstISOLangUpdate" );
			...
			// Read the instance
			ICFIntISOLangObj origBuff = ((ICFIntISOLangObj)schemaObj.getISOLangTableObj().readISOLang( pkey ));
			if( origBuff == null ) {
				throw new CFLibNullArgumentException( getClass(),
					S_ProcName,
					0,
					"getISOLangTableObj().readISOLang()" );
			}
			else {
				// Edit the instance
				ICFIntISOLangEditObj editBuff = (ICFIntISOLangEditObj)origBuff.beginEdit();
				CFSecISOLangBuff dataBuff = editBuff.getISOLangBuff();
			...
			if( updatedAt != null ) {
				...
				String response = schemaFormatter.formatRspnXmlPreamble() + "\n"
					+	"\t" + CFIntXMsgISOLangMessageFormatter.formatISOLangRspnUpdated( "\n\t\t\t", origBuff.getISOLangBuff() )
					+	"\n"
