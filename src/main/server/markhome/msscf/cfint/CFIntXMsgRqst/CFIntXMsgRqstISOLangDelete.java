...
/*
 *	CFIntXMsgRqstISOLangDelete XML SAX Element  implementation
 *	for ISOLang.
 */
public class CFIntXMsgRqstISOLangDelete
	extends CFLibXmlCoreElementHandler
{
	public CFIntXMsgRqstISOLangDelete( CFIntXMsgRqst xmsgRqst ) {
		super( xmsgRqst );
	}
	...
	throws SAXException
	{
		...
		try {
			...
			assert qName.equals( "RqstISOLangDelete" );
			...
			// Delete the object
			ICFIntISOLangObj read = ((ICFIntISOLangObj)schemaObj.getISOLangTableObj().readISOLang( pkey ));
			if( read != null ) {
				if( read.getISOLangBuff().getRequiredRevision() > natRevision ) {
					throw new CFLibCollisionDetectedException( getClass(),
						S_ProcName,
						"Collision detected preparing to delete ISOLang" );
				}
				else {
					ICFIntISOLangEditObj editBuff = (ICFIntISOLangEditObj)read.beginEdit();
					if( editBuff != null ) {
						editBuff.deleteInstance();
						editBuff = null;
						String response = schemaFormatter.formatRspnXmlPreamble() + "\n"
							+	"\t" + CFIntXMsgISOLangMessageFormatter.formatISOLangRspnDeleted() +	"\n"
