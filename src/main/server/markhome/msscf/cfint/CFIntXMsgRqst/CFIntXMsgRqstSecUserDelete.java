...
/*
 *	CFIntXMsgRqstSecUserDelete XML SAX Element  implementation
 *	for SecUser.
 */
public class CFIntXMsgRqstSecUserDelete
	extends CFLibXmlCoreElementHandler
{
	public CFIntXMsgRqstSecUserDelete( CFIntXMsgRqst xmsgRqst ) {
		super( xmsgRqst );
	}
	...
	throws SAXException
	{
		...
		try {
			...
			// Delete the object
			ICFIntSecUserObj read = ((ICFIntSecUserObj)schemaObj.getSecUserTableObj().readSecUser( pkey ));
			if( read != null ) {
				if( read.getSecUserBuff().getRequiredRevision() > natRevision ) {
					...
					ICFIntSecUserEditObj editBuff = (ICFIntSecUserEditObj)read.beginEdit();
					if( editBuff != null ) {
						editBuff.deleteInstance();
						editBuff = null;
						String response = schemaFormatter.formatRspnXmlPreamble() + "\n"
							+	"\t" + CFIntXMsgSecUserMessageFormatter.formatSecUserRspnDeleted() +	"\n"
