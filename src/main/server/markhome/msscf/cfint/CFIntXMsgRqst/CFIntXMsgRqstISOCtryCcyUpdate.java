...
/*
 *	CFIntXMsgRqstISOCtryCcyUpdate XML SAX Element  implementation
 *	for ISOCtryCcy.
 */
public class CFIntXMsgRqstISOCtryCcyUpdate
	extends CFLibXmlCoreElementHandler
{
	public CFIntXMsgRqstISOCtryCcyUpdate( CFIntXMsgRqst xmsgRqst ) {
		super( xmsgRqst );
	}
	...
	throws SAXException
	{
		...
		try {
			...
			assert qName.equals( "RqstISOCtryCcyUpdate" );
			...
			CFSecISOCtryCcyPKey pkey = ((ICFIntSchema)schemaObj.getBackingStore()).getFactoryISOCtryCcy().newPKey();
			...
			// Read the instance
			ICFIntISOCtryCcyObj origBuff = ((ICFIntISOCtryCcyObj)schemaObj.getISOCtryCcyTableObj().readISOCtryCcy( pkey ));
			if( origBuff == null ) {
				throw new CFLibNullArgumentException( getClass(),
					S_ProcName,
					0,
					"getISOCtryCcyTableObj().readISOCtryCcy()" );
			}
			else {
				// Edit the instance
				ICFIntISOCtryCcyEditObj editBuff = (ICFIntISOCtryCcyEditObj)origBuff.beginEdit();
				CFSecISOCtryCcyBuff dataBuff = editBuff.getISOCtryCcyBuff();
			...
			if( updatedAt != null ) {
				...
				String response = schemaFormatter.formatRspnXmlPreamble() + "\n"
					+	"\t" + CFIntXMsgISOCtryCcyMessageFormatter.formatISOCtryCcyRspnUpdated( "\n\t\t\t", origBuff.getISOCtryCcyBuff() )
					+	"\n"
