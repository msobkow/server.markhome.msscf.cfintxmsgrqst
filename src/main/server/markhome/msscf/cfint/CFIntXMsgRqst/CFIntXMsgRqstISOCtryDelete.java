...
/*
 *	CFIntXMsgRqstISOCtryDelete XML SAX Element  implementation
 *	for ISOCtry.
 */
public class CFIntXMsgRqstISOCtryDelete
	extends CFLibXmlCoreElementHandler
{
	public CFIntXMsgRqstISOCtryDelete( CFIntXMsgRqst xmsgRqst ) {
		super( xmsgRqst );
	}
	...
	throws SAXException
	{
		...
		try {
			...
			assert qName.equals( "RqstISOCtryDelete" );
			...
			// Delete the object
			ICFIntISOCtryObj read = ((ICFIntISOCtryObj)schemaObj.getISOCtryTableObj().readISOCtry( pkey ));
			if( read != null ) {
				if( read.getISOCtryBuff().getRequiredRevision() > natRevision ) {
					throw new CFLibCollisionDetectedException( getClass(),
						S_ProcName,
						"Collision detected preparing to delete ISOCtry" );
				}
				else {
					ICFIntISOCtryEditObj editBuff = (ICFIntISOCtryEditObj)read.beginEdit();
					if( editBuff != null ) {
						editBuff.deleteInstance();
						editBuff = null;
						String response = schemaFormatter.formatRspnXmlPreamble() + "\n"
							+	"\t" + CFIntXMsgISOCtryMessageFormatter.formatISOCtryRspnDeleted() +	"\n"
