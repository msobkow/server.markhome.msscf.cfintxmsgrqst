...
// Description: Java 11 XML Message SAX Request Page All Element  for TSecGrpMemb
...
/*
 *	CFIntXMsgRqstTSecGrpMembPageAll XML SAX Element  implementation
 *	for TSecGrpMemb.
 */
public class CFIntXMsgRqstTSecGrpMembPageAll
	extends CFLibXmlCoreElementHandler
{
	public CFIntXMsgRqstTSecGrpMembPageAll( CFIntXMsgRqst xmsgRqst ) {
		super( xmsgRqst );
	}
	...
	throws SAXException
	{
		...
		try {
			...
			assert qName.equals( "RqstTSecGrpMembPageAll" );
			...
			List<ICFSecTSecGrpMembObj> list = schemaObj.getTSecGrpMembTableObj().pageAllTSecGrpMemb( natPriorTenantId,
				natPriorTSecGrpMembId );
			String responseOpening = schemaFormatter.formatRspnXmlPreamble() + "\n"
				+	"\t" + CFIntXMsgTSecGrpMembMessageFormatter.formatTSecGrpMembRspnListOpenTag();
			...
			while( iter.hasNext() ) {
				cur = iter.next();
				subxml = CFIntXMsgTSecGrpMembMessageFormatter.formatTSecGrpMembRspnDerivedRec( "\n\t\t", cur.getTSecGrpMembBuff() );
				xmsgRqst.appendResponse( subxml );
			}
			String responseClosing =
				 	"\n"
				+	"\t" + CFIntXMsgTSecGrpMembMessageFormatter.formatTSecGrpMembRspnListCloseTag()
