...
// Description: Java 11 XML Message SAX Response Delete By Index Element  for SecForm
...
/*
 *	CFIntXMsgRqstSecFormPageBy XML SAX Element  implementation
 *	for SecForm.
 */
public class CFIntXMsgRqstSecFormPageByClusterIdx
	extends CFLibXmlCoreElementHandler
{
	public CFIntXMsgRqstSecFormPageByClusterIdx( CFIntXMsgRqst xmsgRqst ) {
		super( xmsgRqst );
	}
	...
	throws SAXException
	{
		...
		try {
			...
			String attrPriorSecFormId = null;
			...
			assert qName.equals( "RqstSecFormPageByClusterIdx" );
			...
			for( idxAttr = 0; idxAttr < numAttrs; idxAttr++ ) {
				...
				else if( attrLocalName.equals( "PriorSecFormId" ) ) {
					if( attrPriorSecFormId != null ) {
						throw new CFLibUniqueIndexViolationException( getClass(),
							S_ProcName,
							S_LocalName,
							attrLocalName );
					}
					attrPriorSecFormId = attrs.getValue( idxAttr );
				}
			...
			Integer natPriorSecFormId;
			if( ( attrPriorSecFormId == null ) || ( attrPriorSecFormId.length() <= 0 ) ) {
				natPriorSecFormId = null;
			}
			else {
				natPriorSecFormId = Integer.valueOf( Integer.parseInt( attrPriorSecFormId ) );
			}

			// Page the objects
			List<ICFSecSecFormObj> list = schemaObj.getSecFormTableObj().pageSecFormByClusterIdx( natClusterId,
				natPriorClusterId,
				natPriorSecFormId );
			String responseOpening = schemaFormatter.formatRspnXmlPreamble() + "\n"
				+	"\t" + CFIntXMsgSecFormMessageFormatter.formatSecFormRspnListOpenTag();
			xmsgRqst.appendResponse( responseOpening );
			Iterator<ICFSecSecFormObj> iter = list.iterator();
			ICFSecSecFormObj cur;
			String subxml;
			while( iter.hasNext() ) {
				cur = iter.next();
				subxml = CFIntXMsgSecFormMessageFormatter.formatSecFormRspnDerivedRec( "\n\t\t", cur.getSecFormBuff() );
				xmsgRqst.appendResponse( subxml );
			}
			String responseClosing =
				 	"\n"
				+	"\t" + CFIntXMsgSecFormMessageFormatter.formatSecFormRspnListCloseTag()
