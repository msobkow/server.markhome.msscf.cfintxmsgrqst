...
// Description: Java 11 XML Message SAX Response Delete By Index Element  for SysCluster
...
/*
 *	CFIntXMsgRqstSysClusterRdBy XML SAX Element  implementation
 *	for SysCluster.
 */
public class CFIntXMsgRqstSysClusterRdByClusterIdx
	extends CFLibXmlCoreElementHandler
{
	public CFIntXMsgRqstSysClusterRdByClusterIdx( CFIntXMsgRqst xmsgRqst ) {
		super( xmsgRqst );
	}
	...
	throws SAXException
	{
		...
		try {
			...
			assert qName.equals( "RqstSysClusterRdByClusterIdx" );
			...
			// Read the objects
			List<ICFSecSysClusterObj> list = schemaObj.getSysClusterTableObj().readSysClusterByClusterIdx( natClusterId );
			String responseOpening = schemaFormatter.formatRspnXmlPreamble() + "\n"
				+	"\t" + CFIntXMsgSysClusterMessageFormatter.formatSysClusterRspnListOpenTag();
			xmsgRqst.appendResponse( responseOpening );
			Iterator<ICFSecSysClusterObj> iter = list.iterator();
			ICFSecSysClusterObj cur;
			String subxml;
			while( iter.hasNext() ) {
				cur = iter.next();
				subxml = CFIntXMsgSysClusterMessageFormatter.formatSysClusterRspnDerivedRec( "\n\t\t", cur.getSysClusterBuff() );
				xmsgRqst.appendResponse( subxml );
			}
			String responseClosing =
				 	"\n"
				+	"\t" + CFIntXMsgSysClusterMessageFormatter.formatSysClusterRspnListCloseTag()
