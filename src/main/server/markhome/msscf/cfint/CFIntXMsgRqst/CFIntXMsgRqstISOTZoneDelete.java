...
/*
 *	CFIntXMsgRqstISOTZoneDelete XML SAX Element  implementation
 *	for ISOTZone.
 */
public class CFIntXMsgRqstISOTZoneDelete
	extends CFLibXmlCoreElementHandler
{
	public CFIntXMsgRqstISOTZoneDelete( CFIntXMsgRqst xmsgRqst ) {
		super( xmsgRqst );
	}
	...
	throws SAXException
	{
		...
		try {
			...
			assert qName.equals( "RqstISOTZoneDelete" );
			...
			// Delete the object
			ICFIntISOTZoneObj read = ((ICFIntISOTZoneObj)schemaObj.getISOTZoneTableObj().readISOTZone( pkey ));
			if( read != null ) {
				if( read.getISOTZoneBuff().getRequiredRevision() > natRevision ) {
					throw new CFLibCollisionDetectedException( getClass(),
						S_ProcName,
						"Collision detected preparing to delete ISOTZone" );
				}
				else {
					ICFIntISOTZoneEditObj editBuff = (ICFIntISOTZoneEditObj)read.beginEdit();
					if( editBuff != null ) {
						editBuff.deleteInstance();
						editBuff = null;
						String response = schemaFormatter.formatRspnXmlPreamble() + "\n"
							+	"\t" + CFIntXMsgISOTZoneMessageFormatter.formatISOTZoneRspnDeleted() +	"\n"
