...
// Description: Java 11 XML Message SAX Response Delete By Index Element  for SecGroup
...
/*
 *	CFIntXMsgRqstSecGroupRdBy XML SAX Element  implementation
 *	for SecGroup.
 */
public class CFIntXMsgRqstSecGroupRdByClusterVisIdx
	extends CFLibXmlCoreElementHandler
{
	public CFIntXMsgRqstSecGroupRdByClusterVisIdx( CFIntXMsgRqst xmsgRqst ) {
		super( xmsgRqst );
	}
	...
	throws SAXException
	{
		...
		try {
			...
			assert qName.equals( "RqstSecGroupRdByClusterVisIdx" );
			...
			// Read the objects
			List<ICFSecSecGroupObj> list = schemaObj.getSecGroupTableObj().readSecGroupByClusterVisIdx( natClusterId,
				natIsVisible );
			String responseOpening = schemaFormatter.formatRspnXmlPreamble() + "\n"
				+	"\t" + CFIntXMsgSecGroupMessageFormatter.formatSecGroupRspnListOpenTag();
			xmsgRqst.appendResponse( responseOpening );
			Iterator<ICFSecSecGroupObj> iter = list.iterator();
			ICFSecSecGroupObj cur;
			String subxml;
			while( iter.hasNext() ) {
				cur = iter.next();
				subxml = CFIntXMsgSecGroupMessageFormatter.formatSecGroupRspnDerivedRec( "\n\t\t", cur.getSecGroupBuff() );
				xmsgRqst.appendResponse( subxml );
			}
			String responseClosing =
				 	"\n"
				+	"\t" + CFIntXMsgSecGroupMessageFormatter.formatSecGroupRspnListCloseTag()
