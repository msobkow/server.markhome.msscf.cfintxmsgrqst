...
// Description: Java 11 XML Message SAX Response Delete By Index Element  for HostNode
...
/*
 *	CFIntXMsgRqstHostNodeRdBy XML SAX Element  implementation
 *	for HostNode.
 */
public class CFIntXMsgRqstHostNodeRdByHostNameIdx
	extends CFLibXmlCoreElementHandler
{
	public CFIntXMsgRqstHostNodeRdByHostNameIdx( CFIntXMsgRqst xmsgRqst ) {
		super( xmsgRqst );
	}
	...
	throws SAXException
	{
		...
		try {
			...
			assert qName.equals( "RqstHostNodeRdByHostNameIdx" );
			...
			// Read the object
			ICFIntHostNodeObj read = ((ICFIntHostNodeObj)schemaObj.getHostNodeTableObj().readHostNodeByHostNameIdx( natClusterId,
				natHostName,
				true ));
			if( read != null ) {
				String response = schemaFormatter.formatRspnXmlPreamble() + "\n"
					+	"\t" + CFIntXMsgHostNodeMessageFormatter.formatHostNodeRspnSingleOpenTag()
					+	CFIntXMsgHostNodeMessageFormatter.formatHostNodeRspnDerivedRec( "\n\t\t", read.getHostNodeBuff() ) + "\n"
					+	"\t" + CFIntXMsgHostNodeMessageFormatter.formatHostNodeRspnSingleCloseTag()
