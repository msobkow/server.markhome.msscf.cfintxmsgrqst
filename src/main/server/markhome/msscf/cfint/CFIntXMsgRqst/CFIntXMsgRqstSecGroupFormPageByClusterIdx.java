...
/*
 *	CFIntXMsgRqstSecGroupFormPageBy XML SAX Element  implementation
 *	for SecGroupForm.
 */
public class CFIntXMsgRqstSecGroupFormPageByClusterIdx
	extends CFLibXmlCoreElementHandler
{
	public CFIntXMsgRqstSecGroupFormPageByClusterIdx( CFIntXMsgRqst xmsgRqst ) {
		super( xmsgRqst );
	}
	...
	throws SAXException
	{
		...
		try {
			...
			assert qName.equals( "RqstSecGroupFormPageByClusterIdx" );
			...
			// Page the objects
			List<ICFSecSecGroupFormObj> list = schemaObj.getSecGroupFormTableObj().pageSecGroupFormByClusterIdx( natClusterId,
				natPriorClusterId,
				natPriorSecGroupFormId );
			String responseOpening = schemaFormatter.formatRspnXmlPreamble() + "\n"
				+	"\t" + CFIntXMsgSecGroupFormMessageFormatter.formatSecGroupFormRspnListOpenTag();
			...
			while( iter.hasNext() ) {
				cur = iter.next();
				subxml = CFIntXMsgSecGroupFormMessageFormatter.formatSecGroupFormRspnDerivedRec( "\n\t\t", cur.getSecGroupFormBuff() );
				xmsgRqst.appendResponse( subxml );
			}
			String responseClosing =
				 	"\n"
				+	"\t" + CFIntXMsgSecGroupFormMessageFormatter.formatSecGroupFormRspnListCloseTag()
