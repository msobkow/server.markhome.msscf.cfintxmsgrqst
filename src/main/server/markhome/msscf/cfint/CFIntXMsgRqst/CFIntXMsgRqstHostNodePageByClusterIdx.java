...
/*
 *	CFIntXMsgRqstHostNodePageBy XML SAX Element  implementation
 *	for HostNode.
 */
public class CFIntXMsgRqstHostNodePageByClusterIdx
	extends CFLibXmlCoreElementHandler
{
	public CFIntXMsgRqstHostNodePageByClusterIdx( CFIntXMsgRqst xmsgRqst ) {
		super( xmsgRqst );
	}
	...
	throws SAXException
	{
		...
		try {
			...
			assert qName.equals( "RqstHostNodePageByClusterIdx" );
			...
			// Page the objects
			List<ICFSecHostNodeObj> list = schemaObj.getHostNodeTableObj().pageHostNodeByClusterIdx( natClusterId,
				natPriorClusterId,
				natPriorHostNodeId );
			String responseOpening = schemaFormatter.formatRspnXmlPreamble() + "\n"
				+	"\t" + CFIntXMsgHostNodeMessageFormatter.formatHostNodeRspnListOpenTag();
			...
			while( iter.hasNext() ) {
				cur = iter.next();
				subxml = CFIntXMsgHostNodeMessageFormatter.formatHostNodeRspnDerivedRec( "\n\t\t", cur.getHostNodeBuff() );
				xmsgRqst.appendResponse( subxml );
			}
			String responseClosing =
				 	"\n"
				+	"\t" + CFIntXMsgHostNodeMessageFormatter.formatHostNodeRspnListCloseTag()
