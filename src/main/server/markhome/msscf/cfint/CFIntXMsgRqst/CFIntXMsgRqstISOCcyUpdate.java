...
/*
 *	CFIntXMsgRqstISOCcyUpdate XML SAX Element  implementation
 *	for ISOCcy.
 */
public class CFIntXMsgRqstISOCcyUpdate
	extends CFLibXmlCoreElementHandler
{
	public CFIntXMsgRqstISOCcyUpdate( CFIntXMsgRqst xmsgRqst ) {
		super( xmsgRqst );
	}
	...
	throws SAXException
	{
		...
		try {
			...
			assert qName.equals( "RqstISOCcyUpdate" );
			...
			CFSecISOCcyPKey pkey = ((ICFIntSchema)schemaObj.getBackingStore()).getFactoryISOCcy().newPKey();

			short natISOCcyId;
				natISOCcyId = Short.parseShort( attrISOCcyId );
			pkey.setRequiredISOCcyId( natISOCcyId );
			// Read the instance
			ICFIntISOCcyObj origBuff = ((ICFIntISOCcyObj)schemaObj.getISOCcyTableObj().readISOCcy( pkey ));
			if( origBuff == null ) {
				throw new CFLibNullArgumentException( getClass(),
					S_ProcName,
					0,
					"getISOCcyTableObj().readISOCcy()" );
			}
			else {
				// Edit the instance
				ICFIntISOCcyEditObj editBuff = (ICFIntISOCcyEditObj)origBuff.beginEdit();
				CFSecISOCcyBuff dataBuff = editBuff.getISOCcyBuff();
			...
			if( updatedAt != null ) {
				...
				String response = schemaFormatter.formatRspnXmlPreamble() + "\n"
					+	"\t" + CFIntXMsgISOCcyMessageFormatter.formatISOCcyRspnUpdated( "\n\t\t\t", origBuff.getISOCcyBuff() )
					+	"\n"
