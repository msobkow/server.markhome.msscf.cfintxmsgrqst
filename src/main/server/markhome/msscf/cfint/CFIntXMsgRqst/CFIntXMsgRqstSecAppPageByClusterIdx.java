...
/*
 *	CFIntXMsgRqstSecAppPageBy XML SAX Element  implementation
 *	for SecApp.
 */
public class CFIntXMsgRqstSecAppPageByClusterIdx
	extends CFLibXmlCoreElementHandler
{
	public CFIntXMsgRqstSecAppPageByClusterIdx( CFIntXMsgRqst xmsgRqst ) {
		super( xmsgRqst );
	}
	...
	throws SAXException
	{
		...
		try {
			...
			assert qName.equals( "RqstSecAppPageByClusterIdx" );
			...
			// Page the objects
			List<ICFSecSecAppObj> list = schemaObj.getSecAppTableObj().pageSecAppByClusterIdx( natClusterId,
				natPriorClusterId,
				natPriorSecAppId );
			String responseOpening = schemaFormatter.formatRspnXmlPreamble() + "\n"
				+	"\t" + CFIntXMsgSecAppMessageFormatter.formatSecAppRspnListOpenTag();
			...
			while( iter.hasNext() ) {
				cur = iter.next();
				subxml = CFIntXMsgSecAppMessageFormatter.formatSecAppRspnDerivedRec( "\n\t\t", cur.getSecAppBuff() );
				xmsgRqst.appendResponse( subxml );
			}
			String responseClosing =
				 	"\n"
				+	"\t" + CFIntXMsgSecAppMessageFormatter.formatSecAppRspnListCloseTag()
