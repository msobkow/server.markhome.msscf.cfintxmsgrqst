...
// Description: Java 11 XML Message SAX Response Delete By Index Element  for SecApp
...
/*
 *	CFIntXMsgRqstSecAppRdBy XML SAX Element  implementation
 *	for SecApp.
 */
public class CFIntXMsgRqstSecAppRdByUJEEMountIdx
	extends CFLibXmlCoreElementHandler
{
	public CFIntXMsgRqstSecAppRdByUJEEMountIdx( CFIntXMsgRqst xmsgRqst ) {
		super( xmsgRqst );
	}
	...
	throws SAXException
	{
		...
		try {
			...
			assert qName.equals( "RqstSecAppRdByUJEEMountIdx" );
			...
			// Read the object
			ICFIntSecAppObj read = ((ICFIntSecAppObj)schemaObj.getSecAppTableObj().readSecAppByUJEEMountIdx( natClusterId,
				natJEEMountName,
				true ));
			if( read != null ) {
				String response = schemaFormatter.formatRspnXmlPreamble() + "\n"
					+	"\t" + CFIntXMsgSecAppMessageFormatter.formatSecAppRspnSingleOpenTag()
					+	CFIntXMsgSecAppMessageFormatter.formatSecAppRspnDerivedRec( "\n\t\t", read.getSecAppBuff() ) + "\n"
					+	"\t" + CFIntXMsgSecAppMessageFormatter.formatSecAppRspnSingleCloseTag()
