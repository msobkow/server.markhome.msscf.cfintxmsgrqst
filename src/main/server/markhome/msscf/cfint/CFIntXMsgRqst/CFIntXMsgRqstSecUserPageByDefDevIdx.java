...
/*
 *	CFIntXMsgRqstSecUserPageBy XML SAX Element  implementation
 *	for SecUser.
 */
public class CFIntXMsgRqstSecUserPageByDefDevIdx
	extends CFLibXmlCoreElementHandler
{
	public CFIntXMsgRqstSecUserPageByDefDevIdx( CFIntXMsgRqst xmsgRqst ) {
		super( xmsgRqst );
	}
	...
	throws SAXException
	{
		...
		try {
			...
			assert qName.equals( "RqstSecUserPageByDefDevIdx" );
			...
			String natDfltDevName;
			natDfltDevName = attrDfltDevName;
			...
			// Page the objects
			List<ICFSecSecUserObj> list = schemaObj.getSecUserTableObj().pageSecUserByDefDevIdx( natDfltDevUserId,
				natDfltDevName,
				natPriorSecUserId );
			String responseOpening = schemaFormatter.formatRspnXmlPreamble() + "\n"
				+	"\t" + CFIntXMsgSecUserMessageFormatter.formatSecUserRspnListOpenTag();
			...
			while( iter.hasNext() ) {
				cur = iter.next();
				subxml = CFIntXMsgSecUserMessageFormatter.formatSecUserRspnDerivedRec( "\n\t\t", cur.getSecUserBuff() );
				xmsgRqst.appendResponse( subxml );
			}
			String responseClosing =
				 	"\n"
				+	"\t" + CFIntXMsgSecUserMessageFormatter.formatSecUserRspnListCloseTag()
