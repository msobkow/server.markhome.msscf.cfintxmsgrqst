...
/*
 *	CFIntXMsgRqstISOTZoneUpdate XML SAX Element  implementation
 *	for ISOTZone.
 */
public class CFIntXMsgRqstISOTZoneUpdate
	extends CFLibXmlCoreElementHandler
{
	public CFIntXMsgRqstISOTZoneUpdate( CFIntXMsgRqst xmsgRqst ) {
		super( xmsgRqst );
	}
	...
	throws SAXException
	{
		...
		try {
			...
			assert qName.equals( "RqstISOTZoneUpdate" );
			...
			CFSecISOTZonePKey pkey = ((ICFIntSchema)schemaObj.getBackingStore()).getFactoryISOTZone().newPKey();

			short natISOTZoneId;
				natISOTZoneId = Short.parseShort( attrISOTZoneId );
			pkey.setRequiredISOTZoneId( natISOTZoneId );
			// Read the instance
			ICFIntISOTZoneObj origBuff = ((ICFIntISOTZoneObj)schemaObj.getISOTZoneTableObj().readISOTZone( pkey ));
			if( origBuff == null ) {
				throw new CFLibNullArgumentException( getClass(),
					S_ProcName,
					0,
					"getISOTZoneTableObj().readISOTZone()" );
			}
			else {
				// Edit the instance
				ICFIntISOTZoneEditObj editBuff = (ICFIntISOTZoneEditObj)origBuff.beginEdit();
				CFSecISOTZoneBuff dataBuff = editBuff.getISOTZoneBuff();
			...
			if( updatedAt != null ) {
				...
				String response = schemaFormatter.formatRspnXmlPreamble() + "\n"
					+	"\t" + CFIntXMsgISOTZoneMessageFormatter.formatISOTZoneRspnUpdated( "\n\t\t\t", origBuff.getISOTZoneBuff() )
					+	"\n"
