...
// Description: Java 11 XML Message SAX Request Read All Element  for SecGrpInc
...
/*
 *	CFIntXMsgRqstSecGrpIncReadAll XML SAX Element  implementation
 *	for SecGrpInc.
 */
public class CFIntXMsgRqstSecGrpIncReadAll
	extends CFLibXmlCoreElementHandler
{
	public CFIntXMsgRqstSecGrpIncReadAll( CFIntXMsgRqst xmsgRqst ) {
		super( xmsgRqst );
	}
	...
	throws SAXException
	{
		...
		try {
			...
			assert qName.equals( "RqstSecGrpIncReadAll" );
			...
			List<ICFSecSecGrpIncObj> list = schemaObj.getSecGrpIncTableObj().readAllSecGrpInc( true );
			String responseOpening = schemaFormatter.formatRspnXmlPreamble() + "\n"
				+	"\t" + CFIntXMsgSecGrpIncMessageFormatter.formatSecGrpIncRspnListOpenTag();
			...
			while( iter.hasNext() ) {
				cur = iter.next();
				subxml = CFIntXMsgSecGrpIncMessageFormatter.formatSecGrpIncRspnDerivedRec( "\n\t\t", cur.getSecGrpIncBuff() );
				xmsgRqst.appendResponse( subxml );
			}
			String responseClosing =
				 	"\n"
				+	"\t" + CFIntXMsgSecGrpIncMessageFormatter.formatSecGrpIncRspnListCloseTag()
