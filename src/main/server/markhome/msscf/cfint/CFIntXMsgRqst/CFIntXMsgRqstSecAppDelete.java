...
/*
 *	CFIntXMsgRqstSecAppDelete XML SAX Element  implementation
 *	for SecApp.
 */
public class CFIntXMsgRqstSecAppDelete
	extends CFLibXmlCoreElementHandler
{
	public CFIntXMsgRqstSecAppDelete( CFIntXMsgRqst xmsgRqst ) {
		super( xmsgRqst );
	}
	...
	throws SAXException
	{
		...
		try {
			...
			// Delete the object
			ICFIntSecAppObj read = ((ICFIntSecAppObj)schemaObj.getSecAppTableObj().readSecApp( pkey ));
			if( read != null ) {
				if( read.getSecAppBuff().getRequiredRevision() > natRevision ) {
					...
					ICFIntSecAppEditObj editBuff = (ICFIntSecAppEditObj)read.beginEdit();
					if( editBuff != null ) {
						editBuff.deleteInstance();
						editBuff = null;
						String response = schemaFormatter.formatRspnXmlPreamble() + "\n"
							+	"\t" + CFIntXMsgSecAppMessageFormatter.formatSecAppRspnDeleted() +	"\n"
