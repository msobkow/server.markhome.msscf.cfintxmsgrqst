...
/*
 *	CFIntXMsgRqstISOCtryLangRdBy XML SAX Element  implementation
 *	for ISOCtryLang.
 */
public class CFIntXMsgRqstISOCtryLangRdByLangIdx
	extends CFLibXmlCoreElementHandler
{
	public CFIntXMsgRqstISOCtryLangRdByLangIdx( CFIntXMsgRqst xmsgRqst ) {
		super( xmsgRqst );
	}
	...
	throws SAXException
	{
		...
		try {
			...
			assert qName.equals( "RqstISOCtryLangRdByLangIdx" );
			...
			// Read the objects
			List<ICFSecISOCtryLangObj> list = schemaObj.getISOCtryLangTableObj().readISOCtryLangByLangIdx( natISOLangId );
			String responseOpening = schemaFormatter.formatRspnXmlPreamble() + "\n"
				+	"\t" + CFIntXMsgISOCtryLangMessageFormatter.formatISOCtryLangRspnListOpenTag();
			...
			while( iter.hasNext() ) {
				cur = iter.next();
				subxml = CFIntXMsgISOCtryLangMessageFormatter.formatISOCtryLangRspnDerivedRec( "\n\t\t", cur.getISOCtryLangBuff() );
				xmsgRqst.appendResponse( subxml );
			}
			String responseClosing =
				 	"\n"
				+	"\t" + CFIntXMsgISOCtryLangMessageFormatter.formatISOCtryLangRspnListCloseTag()
