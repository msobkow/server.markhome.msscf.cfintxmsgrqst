...
/*
 *	CFIntXMsgRqstSecAppUpdate XML SAX Element  implementation
 *	for SecApp.
 */
public class CFIntXMsgRqstSecAppUpdate
	extends CFLibXmlCoreElementHandler
{
	public CFIntXMsgRqstSecAppUpdate( CFIntXMsgRqst xmsgRqst ) {
		super( xmsgRqst );
	}
	...
	throws SAXException
	{
		...
		try {
			...
			CFSecSecAppPKey pkey = ((ICFIntSchema)schemaObj.getBackingStore()).getFactorySecApp().newPKey();
			...
			// Read the instance
			ICFIntSecAppObj origBuff = ((ICFIntSecAppObj)schemaObj.getSecAppTableObj().readSecApp( pkey ));
			if( origBuff == null ) {
				throw new CFLibNullArgumentException( getClass(),
					S_ProcName,
					0,
				...
				// Edit the instance
				ICFIntSecAppEditObj editBuff = (ICFIntSecAppEditObj)origBuff.beginEdit();
			...
			if( updatedAt != null ) {
				...
				String response = schemaFormatter.formatRspnXmlPreamble() + "\n"
					+	"\t" + CFIntXMsgSecAppMessageFormatter.formatSecAppRspnUpdated( "\n\t\t\t", origBuff.getSecAppBuff() )
					+	"\n"
