...
/*
 *	CFIntXMsgRqstSecGrpMembUpdate XML SAX Element  implementation
 *	for SecGrpMemb.
 */
public class CFIntXMsgRqstSecGrpMembUpdate
	extends CFLibXmlCoreElementHandler
{
	public CFIntXMsgRqstSecGrpMembUpdate( CFIntXMsgRqst xmsgRqst ) {
		super( xmsgRqst );
	}
	...
	throws SAXException
	{
		...
		try {
			...
			assert qName.equals( "RqstSecGrpMembUpdate" );
			...
			CFSecSecGrpMembPKey pkey = ((ICFIntSchema)schemaObj.getBackingStore()).getFactorySecGrpMemb().newPKey();

			long natClusterId;
				natClusterId = Long.parseLong( attrClusterId );
			pkey.setRequiredClusterId( natClusterId );
			long natSecGrpMembId;
				natSecGrpMembId = Long.parseLong( attrSecGrpMembId );
			pkey.setRequiredSecGrpMembId( natSecGrpMembId );
			// Read the instance
			ICFIntSecGrpMembObj origBuff = ((ICFIntSecGrpMembObj)schemaObj.getSecGrpMembTableObj().readSecGrpMemb( pkey ));
			if( origBuff == null ) {
				throw new CFLibNullArgumentException( getClass(),
					S_ProcName,
					0,
					"getSecGrpMembTableObj().readSecGrpMemb()" );
			}
			else {
				// Edit the instance
				ICFIntSecGrpMembEditObj editBuff = (ICFIntSecGrpMembEditObj)origBuff.beginEdit();
				CFSecSecGrpMembBuff dataBuff = editBuff.getSecGrpMembBuff();
			...
			if( updatedAt != null ) {
				...
				String response = schemaFormatter.formatRspnXmlPreamble() + "\n"
					+	"\t" + CFIntXMsgSecGrpMembMessageFormatter.formatSecGrpMembRspnUpdated( "\n\t\t\t", origBuff.getSecGrpMembBuff() )
					+	"\n"
