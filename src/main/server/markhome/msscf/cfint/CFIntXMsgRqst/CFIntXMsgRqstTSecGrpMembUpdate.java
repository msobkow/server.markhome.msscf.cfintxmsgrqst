...
/*
 *	CFIntXMsgRqstTSecGrpMembUpdate XML SAX Element  implementation
 *	for TSecGrpMemb.
 */
public class CFIntXMsgRqstTSecGrpMembUpdate
	extends CFLibXmlCoreElementHandler
{
	public CFIntXMsgRqstTSecGrpMembUpdate( CFIntXMsgRqst xmsgRqst ) {
		super( xmsgRqst );
	}
	...
	throws SAXException
	{
		...
		try {
			...
			assert qName.equals( "RqstTSecGrpMembUpdate" );
			...
			CFSecTSecGrpMembPKey pkey = ((ICFIntSchema)schemaObj.getBackingStore()).getFactoryTSecGrpMemb().newPKey();

			long natTenantId;
				natTenantId = Long.parseLong( attrTenantId );
			pkey.setRequiredTenantId( natTenantId );
			long natTSecGrpMembId;
				natTSecGrpMembId = Long.parseLong( attrTSecGrpMembId );
			pkey.setRequiredTSecGrpMembId( natTSecGrpMembId );
			// Read the instance
			ICFIntTSecGrpMembObj origBuff = ((ICFIntTSecGrpMembObj)schemaObj.getTSecGrpMembTableObj().readTSecGrpMemb( pkey ));
			if( origBuff == null ) {
				throw new CFLibNullArgumentException( getClass(),
					S_ProcName,
					0,
					"getTSecGrpMembTableObj().readTSecGrpMemb()" );
			}
			else {
				// Edit the instance
				ICFIntTSecGrpMembEditObj editBuff = (ICFIntTSecGrpMembEditObj)origBuff.beginEdit();
				CFSecTSecGrpMembBuff dataBuff = editBuff.getTSecGrpMembBuff();
			...
			if( updatedAt != null ) {
				...
				String response = schemaFormatter.formatRspnXmlPreamble() + "\n"
					+	"\t" + CFIntXMsgTSecGrpMembMessageFormatter.formatTSecGrpMembRspnUpdated( "\n\t\t\t", origBuff.getTSecGrpMembBuff() )
					+	"\n"
