...
/*
 *	CFIntXMsgRqstSecGrpIncUpdate XML SAX Element  implementation
 *	for SecGrpInc.
 */
public class CFIntXMsgRqstSecGrpIncUpdate
	extends CFLibXmlCoreElementHandler
{
	public CFIntXMsgRqstSecGrpIncUpdate( CFIntXMsgRqst xmsgRqst ) {
		super( xmsgRqst );
	}
	...
	throws SAXException
	{
		...
		try {
			...
			assert qName.equals( "RqstSecGrpIncUpdate" );
			...
			CFSecSecGrpIncPKey pkey = ((ICFIntSchema)schemaObj.getBackingStore()).getFactorySecGrpInc().newPKey();

			long natClusterId;
				natClusterId = Long.parseLong( attrClusterId );
			pkey.setRequiredClusterId( natClusterId );
			long natSecGrpIncId;
				natSecGrpIncId = Long.parseLong( attrSecGrpIncId );
			pkey.setRequiredSecGrpIncId( natSecGrpIncId );
			// Read the instance
			ICFIntSecGrpIncObj origBuff = ((ICFIntSecGrpIncObj)schemaObj.getSecGrpIncTableObj().readSecGrpInc( pkey ));
			if( origBuff == null ) {
				throw new CFLibNullArgumentException( getClass(),
					S_ProcName,
					0,
					"getSecGrpIncTableObj().readSecGrpInc()" );
			}
			else {
				// Edit the instance
				ICFIntSecGrpIncEditObj editBuff = (ICFIntSecGrpIncEditObj)origBuff.beginEdit();
				CFSecSecGrpIncBuff dataBuff = editBuff.getSecGrpIncBuff();
			...
			if( updatedAt != null ) {
				...
				String response = schemaFormatter.formatRspnXmlPreamble() + "\n"
					+	"\t" + CFIntXMsgSecGrpIncMessageFormatter.formatSecGrpIncRspnUpdated( "\n\t\t\t", origBuff.getSecGrpIncBuff() )
					+	"\n"
