...
// Description: Java 11 XML Message SAX Request Page All Element  for Service
...
/*
 *	CFIntXMsgRqstServicePageAll XML SAX Element  implementation
 *	for Service.
 */
public class CFIntXMsgRqstServicePageAll
	extends CFLibXmlCoreElementHandler
{
	public CFIntXMsgRqstServicePageAll( CFIntXMsgRqst xmsgRqst ) {
		super( xmsgRqst );
	}
	...
	throws SAXException
	{
		...
		try {
			...
			assert qName.equals( "RqstServicePageAll" );
			...
			List<ICFSecServiceObj> list = schemaObj.getServiceTableObj().pageAllService( natPriorClusterId,
				natPriorServiceId );
			String responseOpening = schemaFormatter.formatRspnXmlPreamble() + "\n"
				+	"\t" + CFIntXMsgServiceMessageFormatter.formatServiceRspnListOpenTag();
			...
			while( iter.hasNext() ) {
				cur = iter.next();
				subxml = CFIntXMsgServiceMessageFormatter.formatServiceRspnDerivedRec( "\n\t\t", cur.getServiceBuff() );
				xmsgRqst.appendResponse( subxml );
			}
			String responseClosing =
				 	"\n"
				+	"\t" + CFIntXMsgServiceMessageFormatter.formatServiceRspnListCloseTag()
